...
fun SortScreen(
    ...
    Column(
        ...
        BinaryChooser(
            option1 = "Ascending",
            option2 = "Descending",
            onChange = { sortSettings.favoritesSortType = options[it] },
            startsAt = options.indexOf(sortSettings.favoritesSortType),
        )
        val sortOptions = remember { arrayOf("ID", "Name", "Total", "HP", "Attack", "Defense", "Special Attack", "Special Defense", "Speed") }
        RadioOptions(
            options = sortOptions,
            onSelectionChanged = { sortSettings.favoritesSortMethod = it },
            modifier = Modifier.padding(horizontal = 5.dp),
            selectedInitially = sortSettings.favoritesSortMethod,
        )
